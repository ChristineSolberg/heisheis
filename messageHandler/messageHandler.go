...
func MessageHandler(recvChan chan message.UpdateMessage, sendChan chan message.UpdateMessage, newOrderToFSM chan elevatorStatus.Elevator, elevChan chan elevatorStatus.Elevator, abortElev chan bool, shouldAbort chan bool){ 
	...
	for{
		...
		case message.IAmAlive:
			...
			if shouldAddToMap == true{
				...
				elevatorTimers[msg.ElevatorStatus.IP] = time.AfterFunc(time.Second*2, func() { eventHandler.DeleteElevator(elevators, ip, sendChan, elevatorTimers, myIP, elevChan, abortElev, masterMatrix, shouldAbort)})
		...
		case message.CompletedOrder:
			...
			for button := 0; button < driver.NUM_BUTTONS; button++{
				if completedOrder[button] == 1{
					...
					if button < 2{
						sendChan <-message.UpdateMessage{MessageType: message.LightUpdate, DelOrder: completedOrder, Order: noOrder}
					} else {
						sendChan <-message.UpdateMessage{MessageType: message.LightUpdate, RecieverIP: msg.ElevatorStatus.IP, DelOrder: completedOrder, Order: noOrder}
					}
				}
			}
