...
func main() {
	...
	deleteCompletedOrder := make(chan [4]int, 100)
	powerOffDetected := make(chan bool)
	shouldAbort := make(chan bool)
	abortElev := make(chan bool)

	go messageHandler.MessageHandler(recvNetwork, sendNetwork, newOrderToFSM, elevObject, abortElev, shouldAbort)
	go eventHandler.EventHandler(newStateUpdate, buttonPushed, powerOffDetected, deleteCompletedOrder, elevObject, sendNetwork, notAlive, shouldAbort)
